...
4. If the user gave no explicit task but attached planning or implementation docs, ask with `questions` when the next bounded task is not clear enough to choose confidently. Only hand off to `Planner Agent` to derive the next bounded candidate when the docs support a clear recommendation. If the docs are already exhausted, say so directly. Label any extra ideas as suggestions.
5. Use `questions` when scope, approval, or success criteria are unclear. Summarize the stage first, explain the decision in plain language, name affected files or behaviors, recommend a default when one exists, and keep freeform input enabled.
...
7. For multi-stage work, create a short `#todo` plan, but keep only one specialist active at a time.
...
10. If a planning document drives the work, have the owning specialist update that same document with the selected task, clarifications, and scope changes before coding, and record implementation status and any follow-up notes there before review.
...
If research is the active stage, stay in research mode unless the user explicitly expands scope into implementation. Prefer `Web Research Agent` for upstream facts and `Explorer Agent` for repository-local questions.
