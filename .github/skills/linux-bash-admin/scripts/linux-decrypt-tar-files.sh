...
mapfile -d '' files < <(find "$SOURCE_PATH" -maxdepth 1 -type f -name '*.enc' -print0)
...
if (( total == 0 )); then
  echo "No .enc files found under source path." >&2
fi

for i in "${!files[@]}"; do
  file="${files[$i]}"
  base="$(basename "$file")"
  output_name="${base%.enc}"
  output="$TARGET_PATH/$output_name"
  ...
  if [[ "$MODE" == "dry-run" ]]; then
    echo "  dry-run: would decrypt $file -> $output"
    success=$((success + 1))
    continue
  fi
  ...
  if openssl enc -d -aes-256-cbc -pbkdf2 -in "$file" -out "$output" -pass "pass:$PASSPHRASE"; then
    ...
    rm -f "$output"
    echo "  fail: could not decrypt $file (partial output removed)" >&2
    failed=$((failed + 1))
  fi
...
echo "  Total encrypted files: $total"
