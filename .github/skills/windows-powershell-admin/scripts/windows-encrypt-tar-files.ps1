...
$tarFiles = Get-ChildItem -Path $resolvedSource -File -Filter *.tar -ErrorAction Stop
...
for ($index = 0; $index -lt $total; $index++) {
    $file = $tarFiles[$index]
    $targetFile = Join-Path -Path $resolvedTarget -ChildPath ("{0}.enc" -f $file.Name)

    Write-Progress -Id 2 -Activity "Encrypting tar files" -Status ("{0} ({1}/{2})" -f $file.Name, ($index + 1), $total) -PercentComplete ((($index + 1) / [Math]::Max($total, 1)) * 100)

    if ((Test-Path -Path $targetFile) -and -not $Overwrite) {
        Write-Warning "Encrypted output exists, skipping without -Overwrite: $targetFile"
        $failed++
    ...
    if ($WhatIf) {
        Write-Output "WhatIf: would encrypt $($file.FullName) to $targetFile"
        $success++
        $results += [PSCustomObject]@{
            Source = $file.FullName
            Output = $targetFile
    ...
    try {
        ...
        & $openssl.Source enc -aes-256-cbc -pbkdf2 -salt -in $file.FullName -out $targetFile -pass ("pass:{0}" -f $Passphrase)
        ...
        $failed++
        Write-Warning ("Failed to encrypt '{0}': {1}" -f $file.FullName, $_.Exception.Message)
        $results += [PSCustomObject]@{
            Source = $file.FullName
            Output = $targetFile
            SourceBytes = $file.Length
...
Write-Progress -Id 2 -Activity "Encrypting tar files" -Completed
