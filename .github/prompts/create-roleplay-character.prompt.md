...
9. If the user wants a one-off character packet artifact that stays inside this creative subsystem, create or revise the narrowest appropriate file directly. If the work becomes a reusable `.github` wrapper or otherwise broadens into prompt, agent, instruction, or workflow-customization authoring or refactoring, use `Meta Agent` as the next owner.
...
- State whether the task is packet design, packet normalization, one-off packet artifact creation, reusable customization-wrapper creation, or start-roleplay.
