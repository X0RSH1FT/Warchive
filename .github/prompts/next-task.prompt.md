...
4. Canonical `#askQuestions` rule: when clarification or approval is needed, summarize the requested stage first, explain the decision and why it matters in plain language, include affected files/modules/behaviors, recommend a default when one exists, rank options when useful, and keep freeform input enabled.
5. Canonical validation and review rule for code work: require changed or added modules to pass relevant tests and quality gates before review, route to `Testing Agent` when validation depth is the main remaining risk, and route to `Reviewer Agent` for non-trivial scope unless the user explicitly opts out.
6. If the user did not provide an explicit task but attached planning or implementation documents:
   ...
   - when using `#askQuestions`, follow the canonical rule
...
7. If scope or success criteria are ambiguous, use `#askQuestions` before dispatching and follow the canonical rule.
8. Decide whether the selected task is primarily planning, implementation, system-administration, interface-design, creative-direction, prompt-workflow, documentation, testing, review, or research.
9. Prefer `Planner Agent` when the task is ambiguous, cross-cutting, or driven by planning docs with multiple plausible next slices. Ask it for a bounded brief with target files, acceptance criteria, risks, scope exclusions, and the first validation step.
10. Build a short execution plan with `#todo` when the task spans multiple stages.
11. Dispatch to the most appropriate specialist:
   ...
   - `System Administration Agent` for Windows or Linux operational diagnostics, disk or filesystem tasks, service or process triage, and bounded maintenance commands with explicit safety controls
