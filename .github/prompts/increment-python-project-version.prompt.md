...
2. Start by identifying the target project root and the authoritative `pyproject.toml` that owns `[project].version`.
3. If the target path or desired version change is unclear, use `#askQuestions` before dispatching. Ask whether the user wants an exact version or a semantic bump (`patch`, `minor`, or `major`), explain why the choice matters, recommend the smallest sensible bump by default, and keep freeform input enabled.
4. If the repository has multiple plausible version surfaces, treat `pyproject.toml` package metadata as the source of truth unless the user explicitly says otherwise.
5. Route to `Implementation Agent` for the edit once the target version is clear.
